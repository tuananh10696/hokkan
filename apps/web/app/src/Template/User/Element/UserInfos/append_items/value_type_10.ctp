<tr id="append_block-<?= $append['slug'] ?>">
    ...
    <?php if($append['is_required'] == 1):?>
        <?= $this->Form->select("info_append_items.{$num}.value_int", $list,['style' => 'max-width:200px;', 'default' => $value_default]) ?>
    <?php else:?>
        <?= $this->Form->select("info_append_items.{$num}.value_int", $list, ['empty' => '--', 'style' => 'max-width:200px;', 'default' => $value_default]) ?>
    <?php endif;?>
